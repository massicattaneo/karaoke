/*
 ...
 Output Object Specs:
 MIDIObject{
 formatType: 0|1|2,              // 0 - one, single multi-channel track, 1 - one or more simultaneous tracks, 2 - one or more sequentially independent single-track patterns
 timeDivision: (int),
 tracks: (int),
 track: Array[
 [0]: Object{
...
JSMIDIParser = {
    ...
    parse: function (FileAsUint8Array) {
        ...
        console.log("time byte 1,2", timeDivisionByte1, timeDivisionByte2);
        ...
        } else MIDI.timeDivision = (timeDivisionByte1 * 256) + timeDivisionByte2;// else... ticks per beat MODE  (2 bytes value) -> division of a quarter-note
        ...
        for (var t = 1; t <= MIDI.tracks; t++) {
            ...
            while (!endOfTrack) {
                ...
                if (statusByte == 0xFF) { 										// Meta Event type
                    ...
                    //console.log("MIDI Control Events type: -> " + MIDI.track[t - 1].event[e - 1].type);
