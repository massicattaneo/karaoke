var GUI = function () {
    ...
    document.addEventListener("mouseup", function (e) {
        ...
        if (togglePressed && toggleDragged) {
            ...
            if (openHeight > controllerHeight) {
                ...
                if (controllerContainer.children.length >= 1) {
    ...
    this.add = function () {
        ...
        if (type != "function" && GUI.saveIndex < GUI.savedValues.length) {
...
GUI.load = function (c) {
    var d = c.split(",");
    var a = parseInt(d[0]);
    GUI.scrollTop = parseInt(d[1]);
    for (var b = 0; b < a; b++) {
        var e = d.splice(2, 4);
        GUI.savedAppearanceVars.push(e)
    }
    GUI.savedValues = d.splice(2, d.length)
};
GUI.savedValues = [];
GUI.savedAppearanceVars = [];
GUI.getSaveString = function () {
    var d = [];
    d.push(GUI.allGuis.length);
    d.push(document.body.scrollTop);
    for (var c in GUI.allGuis) {
        var e = GUI.allGuis[c].appearanceVars();
        for (var b = 0; b < e.length; b++) {
            d.push(e[b])
        }
    }
    for (var c in GUI.allControllers) {
        if (GUI.allControllers[c].type == "function") {
            continue
        }
        var a = GUI.allControllers[c].getValue();
        if (GUI.allControllers[c].type == "number") {
            a = GUI.roundToDecimal(a, 4)
        }
        d.push(a)
    }
    return d.join(",")
};
GUI.getVarFromURL = function (a) {
    var e = [], d;
...
GUI.replaceGetVar = function (g, f) {
    var d = [], c;
...
GUI.makeUnselectable = function (a) {
...
GUI.map = function (a, d, b, e, c) {
    var a = e + (c - e) * ((a - d) / (b - d));
    return a
};
GUI.constrain = function (a, c, b) {
    if (a < c) {
        a = c
    } else {
        if (a > b) {
...
GUI.error = function (a) {
    if (typeof console.error == "function") {
        console.error("[GUI ERROR] " + a)
    }
};
GUI.roundToDecimal = function (c, a) {
    var b = Math.pow(10, a);
    return Math.round(c * b) / b
};
GUI.extendController = function (a) {
    a.prototype = new GUI.Controller();
    a.prototype.constructor = a
};
if (GUI.getVarFromURL("saveString") != null) {
    GUI.load(GUI.getVarFromURL("saveString"))
}
GUI.Slider = function (a, d, i, b, h) {
    var d = d;
    var i = i;
    var b = b;
    var g = false;
    ...
    this.__defineSetter__("value", function (m) {
        var l = GUI.map(m, d, i, 0, 100);
        this.fg.style.width = l + "%"
    });
    var c = function (l) {
        if (!g) {
            return
        }
        var n = f(e.domElement);
        var m = GUI.map(l.pageX, n[0], n[0] + e.domElement.offsetWidth, d, i);
        m = Math.round(m / b) * b;
        a.setValue(m)
    };
    this.domElement.addEventListener("mousedown", function (l) {
        g = true;
        j = k = l.pageX;
        e.domElement.setAttribute("class", "guidat-slider-bg active");
        e.fg.setAttribute("class", "guidat-slider-fg active");
        c(l)
    }, false);
    document.addEventListener("mouseup", function (l) {
        e.domElement.setAttribute("class", "guidat-slider-bg");
        e.fg.setAttribute("class", "guidat-slider-fg");
        g = false
    }, false);
...
GUI.Controller = function () {
    ...
    if (arguments.length > 0) {
        this.initialValue = this.propertyName[this.object]
    }
    this.domElement = document.createElement("div");
...
GUI.Controller.prototype.name = function (a) {
...
GUI.Controller.prototype.setValue = function (a) {
    this.object[this.propertyName] = a;
    if (this.changeFunction != null) {
        this.changeFunction.call(this, a)
    }
    this.updateDisplay();
    return this
};
...
GUI.Controller.prototype.onChange = function (a) {
...
GUI.NumberController = function () {
    ...
    c.setAttribute("id", this.propertyName);
    ...
    var b;
    if (e != undefined && j != undefined) {
        b = new GUI.Slider(this, e, j, d, this.getValue());
        this.domElement.appendChild(b.domElement)
    }
    c.addEventListener("blur", function (k) {
    ...
    c.addEventListener("mousewheel", function (k) {
        k.preventDefault();
        f.setValue(f.getValue() + Math.abs(k.wheelDeltaY) / k.wheelDeltaY * d);
        return false
    }, false);
    c.addEventListener("mousedown", function (k) {
        py = i = k.pageY;
        g = true;
        document.addEventListener("mousemove", h, false)
    }, false);
    c.addEventListener("keydown", function (l) {
        switch (l.keyCode) {
            case 38:
                var k = f.getValue() + d;
                f.setValue(k);
    ...
    document.addEventListener("mouseup", function (k) {
        document.removeEventListener("mousemove", h, false);
        GUI.makeSelectable(f.parent.domElement);
        GUI.makeSelectable(c);
        if (g && !a) {
            c.focus();
            c.select()
        }
    ...
    var h = function (m) {
        a = true;
        m.preventDefault();
        GUI.makeUnselectable(f.parent.domElement);
        GUI.makeUnselectable(c);
        py = i;
        i = m.pageY;
        var k = py - i;
        var l = f.getValue() + k * d;
        f.setValue(l);
        return false
    };
    this.setValue = function (k) {
        ...
        return GUI.Controller.prototype.setValue.call(this, k)
    };
    this.updateDisplay = function () {
        c.value = GUI.roundToDecimal(f.getValue(), 4);
        if (b) {
            b.value = f.getValue()
        }
    }
